...
object Backtrack : Module("Backtrack", Category.COMBAT) {
    ...
    private val nextBacktrackDelay by int("NextBacktrackDelay", 0, 0..1000000) { mode == "Modern" }
    private val maxDelay: Value<Int> = int("MaxDelay", 80, 0..1000000).onChange { _, new ->
        new.coerceAtLeast(minDelay.get())
    }
    private val minDelay: Value<Int> = int("MinDelay", 80, 0..1000000) {
        mode == "Modern"
    }.onChange { _, new ->
